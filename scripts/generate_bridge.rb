...
def generate_cpp_bridge(specs)
  ...
  lines << '  QMetaObject::Connection connection;'
  lines << '};'
  lines << ''
  ...
  lines << '  const QByteArray signature = signal_method.methodSignature();'
  lines << ''
  lines << '  QMetaObject::Connection connection;'
  lines << ''
  lines << '  if (signature == "clicked(bool)" || signature == "clicked()") {'
  lines << '    auto* button = qobject_cast<QAbstractButton*>(obj);'
  lines << '    if (!button) {'
  lines << '      return -3;'
  lines << '    }'
  lines << '    connection = QObject::connect(button, &QAbstractButton::clicked, button, [obj, signal_index](bool checked) {'
  lines << '      if (!signal_callback_ref()) {'
  lines << '        return;'
  lines << '      }'
  lines << '      signal_callback_ref()(obj, signal_index, checked ? "1" : "0");'
  lines << '    });'
  lines << '  } else if (signature == "pressed()") {'
  lines << '    auto* button = qobject_cast<QAbstractButton*>(obj);'
  lines << '    if (!button) {'
  lines << '      return -3;'
  lines << '    }'
  lines << '    connection = QObject::connect(button, &QAbstractButton::pressed, button, [obj, signal_index]() {'
  lines << '      if (!signal_callback_ref()) {'
  lines << '        return;'
  lines << '      }'
  lines << '      signal_callback_ref()(obj, signal_index, nullptr);'
  lines << '    });'
  lines << '  } else if (signature == "released()") {'
  lines << '    auto* button = qobject_cast<QAbstractButton*>(obj);'
  lines << '    if (!button) {'
  lines << '      return -3;'
  lines << '    }'
  lines << '    connection = QObject::connect(button, &QAbstractButton::released, button, [obj, signal_index]() {'
  lines << '      if (!signal_callback_ref()) {'
  lines << '        return;'
  lines << '      }'
  lines << '      signal_callback_ref()(obj, signal_index, nullptr);'
  lines << '    });'
  lines << '  } else if (signature == "toggled(bool)") {'
  lines << '    auto* button = qobject_cast<QAbstractButton*>(obj);'
  lines << '    if (!button) {'
  lines << '      return -3;'
  lines << '    }'
  lines << '    connection = QObject::connect(button, &QAbstractButton::toggled, button, [obj, signal_index](bool checked) {'
  lines << '      if (!signal_callback_ref()) {'
  lines << '        return;'
  lines << '      }'
  lines << '      signal_callback_ref()(obj, signal_index, checked ? "1" : "0");'
  lines << '    });'
  lines << '  } else if (signature == "textChanged(QString)") {'
  lines << '    auto* line_edit = qobject_cast<QLineEdit*>(obj);'
  lines << '    if (!line_edit) {'
  lines << '      return -3;'
  lines << '    }'
  lines << '    connection = QObject::connect(line_edit, &QLineEdit::textChanged, line_edit, [obj, signal_index](const QString& value) {'
  lines << '      if (!signal_callback_ref()) {'
  lines << '        return;'
  lines << '      }'
  lines << '      thread_local QByteArray utf8;'
  lines << '      utf8 = value.toUtf8();'
  lines << '      signal_callback_ref()(obj, signal_index, utf8.constData());'
  lines << '    });'
  lines << '  } else if (signature == "textEdited(QString)") {'
  lines << '    auto* line_edit = qobject_cast<QLineEdit*>(obj);'
  lines << '    if (!line_edit) {'
  lines << '      return -3;'
  lines << '    }'
  lines << '    connection = QObject::connect(line_edit, &QLineEdit::textEdited, line_edit, [obj, signal_index](const QString& value) {'
  lines << '      if (!signal_callback_ref()) {'
  lines << '        return;'
  lines << '      }'
  lines << '      thread_local QByteArray utf8;'
  ...
  lines << '      signal_callback_ref()(obj, signal_index, utf8.constData());'
  lines << '    });'
  lines << '  } else if (signature == "editingFinished()") {'
  lines << '    auto* line_edit = qobject_cast<QLineEdit*>(obj);'
  lines << '    if (!line_edit) {'
  lines << '      return -3;'
  lines << '    }'
  lines << '    connection = QObject::connect(line_edit, &QLineEdit::editingFinished, line_edit, [obj, signal_index]() {'
  lines << '      if (!signal_callback_ref()) {'
  lines << '        return;'
  lines << '      }'
  lines << '      signal_callback_ref()(obj, signal_index, nullptr);'
  lines << '    });'
  lines << '  } else if (signature == "returnPressed()") {'
  lines << '    auto* line_edit = qobject_cast<QLineEdit*>(obj);'
  lines << '    if (!line_edit) {'
  lines << '      return -3;'
  lines << '    }'
  lines << '    connection = QObject::connect(line_edit, &QLineEdit::returnPressed, line_edit, [obj, signal_index]() {'
  lines << '      if (!signal_callback_ref()) {'
  lines << '        return;'
  lines << '      }'
  lines << '      signal_callback_ref()(obj, signal_index, nullptr);'
  ...
  lines << '  by_index[signal_index].push_back(QtRubySignalHandler{signal_index, connection});'
  ...
  lines << '        QObject::disconnect(handler.connection);'
