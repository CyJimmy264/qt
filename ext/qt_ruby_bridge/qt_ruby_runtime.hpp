...
inline EventCallback& event_callback_ref() {
  static EventCallback callback = nullptr;
  return callback;
}

inline SignalCallback& signal_callback_ref() {
...
inline KeyStateFilter* key_filter_instance() {
  static KeyStateFilter filter;
  return &filter;
}

inline void ensure_key_filter_installed() {
...
inline std::unordered_map<QObject*, std::unordered_set<int>>& watched_events() {
...
inline std::unordered_map<QObject*, SignalHandlersByIndex>& signal_handlers() {
  static std::unordered_map<QObject*, SignalHandlersByIndex> handlers;
  return handlers;
}

inline int resolve_signal_index(QObject* obj, const char* signal_name) {
  if (!obj || !signal_name) {
    return -1;
  }
...
inline EventFilter* event_filter_instance() {
  static EventFilter filter;
  return &filter;
}

inline void ensure_event_filter_installed() {
  static bool installed = false;
  if (!installed && qApp) {
...
inline void qapplication_process_events() { QCoreApplication::processEvents(); }

inline int qapplication_top_level_widgets_count() {
  return QApplication::topLevelWidgets().size();
}

inline int qapplication_mouse_x() { return QCursor::pos().x(); }
inline int qapplication_mouse_y() { return QCursor::pos().y(); }
inline int qapplication_mouse_buttons() { return static_cast<int>(QGuiApplication::mouseButtons()); }

inline int qapplication_key_down(int key) {
...
inline void set_event_callback(void* callback_ptr) {
  event_callback_ref() = reinterpret_cast<EventCallback>(callback_ptr);
  ensure_event_filter_installed();
}

inline void watch_qobject_event(void* object_handle, int event_type) {
  if (!object_handle) {
    return;
  }
...
inline void unwatch_qobject_event(void* object_handle, int event_type) {
  if (!object_handle) {
    return;
  }
  auto* obj = static_cast<QObject*>(object_handle);
...
inline void set_signal_callback(void* callback_ptr) {
  signal_callback_ref() = reinterpret_cast<SignalCallback>(callback_ptr);
}

inline int qobject_connect_signal(void* object_handle, const char* signal_name) {
...
inline int qobject_disconnect_signal(void* object_handle, const char* signal_name) {
  if (!object_handle) {
    return -1;
  }
...
inline int qwidget_map_from_global_x(void* handle, int gx, int gy) {
  if (!handle) {
    return 0;
  }
...
inline int qwidget_map_from_global_y(void* handle, int gx, int gy) {
  if (!handle) {
    return 0;
  }
  auto* widget = static_cast<QWidget*>(handle);
